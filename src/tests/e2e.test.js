...
describe("Purchase flow", () => {
  ...
  it("Make checkout on screen 'Your cart'", async () => {
    ...
    expect(await firstCheckoutPage.getTitle()).equal(
  ...
  it("Make checkout overview", async () => {
    ...
    expect(await secondCheckoutPage.getInfoLabel(0)).equal(
      "Payment Information:"
    );
    expect(await secondCheckoutPage.getInfoValue(0)).equal("SauceCard #31337");
    expect(await secondCheckoutPage.getInfoLabel(1)).equal(
      "Shipping Information:"
    );
    expect(await secondCheckoutPage.getInfoValue(1)).equal(
    ...
    expect(await secondCheckoutPage.getTotalPrice()).equal(
      `Total: $${((extractPrice + extractTax) * 100) / 100}`
    );
  ...
  it("Checkout complete", async () => {
    expect(await thirdCheckoutPage.getPrimaryDescription()).equal(
      "THANK YOU FOR YOUR ORDER"
    );
    expect(await thirdCheckoutPage.getSecondaryDescription()).equal(
